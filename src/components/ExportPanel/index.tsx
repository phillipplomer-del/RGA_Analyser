...
export function ExportPanel({ analysis, chartRef, compact = false }: ExportPanelProps) {
  ...
  const content = (
    <div className={`flex ${compact ? 'flex-col' : 'flex-wrap'} gap-3`}>
      <Button onClick={handleExportPDF} variant="primary" disabled={isExportingPDF} className={compact ? 'w-full justify-center' : ''}>
        {isExportingPDF ? (
      ...
      <Button onClick={handleExportHTML} variant="secondary" className={compact ? 'w-full justify-center' : ''}>
        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <Button onClick={handleExportCSV} variant="ghost" className={compact ? 'w-full justify-center' : ''}>
        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
